...
# --- 階級リスト（全100段階のイメージで主要な節目を定義） ---
# 100個書くのは大変なので、リスト外のインデックスは自動で埋めるか、節目だけ定義します
rank_names = [
    "農民", "迷子の足軽", "いけてる足軽", "槍の又左のパシリ", "運のいい足軽頭", # Lv 0-4
    "影武者の影武者", "疾風の忍び", "闇夜の暗殺者", "独眼竜の右目", "傾奇者", # Lv 5-9
    "六文銭の旗持ち", "信濃の荒武者", "川中島からの生還者", "赤備えの先鋒", "表裏比興の弟子", # Lv 10-14
    "上田城の門番", "真田十勇士", "真田信之級", "鬼神の如き猛将", "越後の龍の鱗", # Lv 15-19
    "剣聖", "東国無双", "天下布武の右腕", "日ノ本一の兵", "覇王の軍師", # Lv 20-24
    "関白", "征夷大将軍", "東照大権現", "漆黒の堕天使", "輪廻転生せし修羅", # Lv 25-29
    # ... 中間は「熟練の〇〇」などで補完可能
]

# --- 後半ほど上がりづらいレベル計算ロジック ---
def calculate_level(score):
    if score <= 40: # Lv 20までは 2問 = 1Lv
        level = score // 2
    elif score <= 130: # Lv 50までは 3問 = 1Lv (40 + 30*3)
        level = 20 + (score - 40) // 3
    elif score <= 290: # Lv 90までは 4問 = 1Lv (130 + 40*4)
        level = 50 + (score - 130) // 4
    else: # Lv 100までは 5問 = 1Lv (290 + 10*5 = 340問で創造主)
        level = 90 + (score - 290) // 5
